...
router.put('/:id', authMiddleware, requireRole('shipper'), (req, res) => {
  ...
  db.run(
    'UPDATE loads SET status = COALESCE(?, status), price_fixed = COALESCE(?, price_fixed), updated_at = CURRENT_TIMESTAMP WHERE id = ? AND shipper_id = ?',
...
router.post('/:id/bids', authMiddleware, requireRole('driver', 'fleet_owner'), (req, res) => {
  ...
  if (proposed_price == null) return res.status(400).json({ error: 'proposed_price required' });
