...
// Get payment by ID (only payer or payee can view)
router.get('/:id', authMiddleware, (req, res) => {
  db.get('SELECT * FROM payments WHERE id = ? AND (payer_id = ? OR payee_id = ? OR ? = "admin")', 
    [req.params.id, req.user.id, req.user.id, req.user.role], (err, row) => {
    if (err) return res.status(500).json({ error: err.message });
...
// Confirm payment (only the payer or admin can confirm)
router.post('/:id/confirm', authMiddleware, (req, res) => {
  db.run(
    'UPDATE payments SET status = ? WHERE id = ? AND (payer_id = ? OR ? = "admin")',
    ['paid', req.params.id, req.user.id, req.user.role],
    function(err) {
      if (err) return res.status(500).json({ error: err.message });
      if (this.changes === 0) return res.status(404).json({ error: 'Payment not found or unauthorized' });
