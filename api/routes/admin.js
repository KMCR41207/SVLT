...
router.get('/users', authMiddleware, requireRole('admin', 'fleet_owner'), (req, res) => {
  const { role } = req.query;
  if (role) {
    db.all('SELECT id, email, name, role, is_verified, phone, created_at FROM users WHERE role = ?', [role], (err, rows) => {
      if (err) return res.status(500).json({ error: err.message });
      res.json(rows || []);
    });
  } else {
    db.all('SELECT id, email, name, role, is_verified, phone, created_at FROM users', [], (err, rows) => {
      if (err) return res.status(500).json({ error: err.message });
      res.json(rows || []);
    });
  }
...
router.get('/trucks', authMiddleware, requireRole('admin'), (req, res) => {
  db.all(
    `SELECT t.*, c.owner_user_id as owner_id FROM trucks t LEFT JOIN companies c ON t.company_id = c.id ORDER BY t.created_at DESC`,
    [],
    (err, rows) => {
      if (err) return res.status(500).json({ error: err.message });
      res.json(rows || []);
...
// Get dashboard stats
router.get('/dashboard/stats', authMiddleware, requireRole('admin', 'shipper', 'driver', 'fleet_owner'), (req, res) => {
  const stats = {};
