...
// Update truck (fleet owner only — must own the truck)
router.put('/:id', authMiddleware, requireRole('fleet_owner'), (req, res) => {
  const { status, assigned_driver_id } = req.body;
  
  db.run(
    'UPDATE trucks SET status = COALESCE(?, status), assigned_driver_id = COALESCE(?, assigned_driver_id) WHERE id = ? AND company_id = (SELECT id FROM companies WHERE owner_user_id = ?)',
    [status || null, assigned_driver_id || null, req.params.id, req.user.id],
    function(err) {
      if (err) return res.status(500).json({ error: err.message });
      if (this.changes === 0) return res.status(404).json({ error: 'Truck not found or unauthorized' });
      res.json({ ok: true });
    }
  );
});
